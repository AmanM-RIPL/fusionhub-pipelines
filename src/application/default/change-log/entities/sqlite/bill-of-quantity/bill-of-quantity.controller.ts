...
export class BillOfQuantityController implements IProjectEntityBaseController {
  validate(changeLog: Selectable<IChangeLog>): void {

    if (!changeLog.entitySchema || typeof changeLog.entitySchema !== "object") {
      const newEntitySchema: Selectable<IBillOfQuantity> = {
        id: 0,
        global_id: 0,
    ...
    const entitySchema = changeLog.entitySchema as Selectable<IBillOfQuantity>;
