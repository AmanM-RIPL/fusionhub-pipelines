...
layout (location = 1) in int edgeIndex;
...
uniform isamplerBuffer edgeDataInt; // data is struct {start_vertex, end_vertex, material_index, dash}
uniform samplerBuffer edgeDataFloat; // data is struct {width, dash_length, gap_length, padding = 0.0f}
...
void main()
{
    // 1. extract edge data from edgeDataInt and edgeDataFloat
    ivec4 edgeInt = texelFetch(edgeDataInt, edgeIndex);
    vec4 edgeFloat = texelFetch(edgeDataFloat, edgeIndex);

    int start_vertex_index = edgeInt.x;
    int end_vertex_index = edgeInt.y;
    int materialIndex = edgeInt.z;
    int edgeDash = edgeInt.w;

    float edgeWidth = edgeFloat.x;
    float edgeDashLength = edgeFloat.y;
    float edgeGapLength = edgeFloat.z;

    // 2. extract the verticies
    vec3 position0 = texelFetch(vertices, int(start_vertex_index)).xyz;
    vec3 position1 = texelFetch(vertices, int(end_vertex_index)).xyz;

    // 3. extract the model matrix (both edge's vertices will have the same model matrix
    int modelMatrixIndex = texelFetch(modelMatrixIndexBuffer, int(start_vertex_index)).x;
    // int modelMatrixIndex = texelFetch(modelMatrixIndexBuffer, 0).x;

    // 3.1 A mat4 takes 4 vec4 slots. We must calculate the starting offset.
    int baseIndex = modelMatrixIndex * 4;

    // 3.2 Fetch the 4 columns/rows of the matrix manually
    ...
    // 4. project vertices to clip space
